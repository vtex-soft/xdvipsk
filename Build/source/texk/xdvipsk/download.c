 ...
 */
#ifdef XDVIPSK
char **getEncoding(char *encoding, char **builtin) {
    int ii;
#else
static char **getEncoding(char *encoding) {
   ...
   if (p == 0) {
      ...
      if (builtin)
      {
          p->glyphs = (char**)mymalloc(256 * sizeof(char*));
          assert(p->glyphs);
          for (ii = 0; ii < 256; ii++)
            p->glyphs[ii] = xstrdup(builtin[ii]);
      }
      else
#endif /* XDVIPSK */
...
downpsfont(charusetype *p, charusetype *all)
{
    ...
    for (; all->fd; all++) {
	   ...
	   char **glyphs = getEncoding(curfnt->resfont->Vectfile);
           c = curfnt->chardesc + 255;
#else /* XDVIPSK */
	   char **glyphs = getEncoding(curfnt->resfont->Vectfile, NULL);
#endif /* XDVIPSK */
           cc = 255;
#ifdef XDVIPSK
           c = find_chardesc(curfnt, cc);
